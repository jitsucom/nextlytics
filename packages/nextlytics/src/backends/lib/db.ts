import type { ClientContext, NextlyticsEvent } from "../../types";
...
export const tableColumns = [
  ...
  { name: "locale", pgType: "TEXT", chType: "Nullable(LowCardinality(String))" },
  { name: "server_context", pgType: "JSONB", chType: "String" },
  { name: "client_context", pgType: "JSONB", chType: "Nullable(String)" },
  { name: "user_traits", pgType: "JSONB", chType: "Nullable(String)" },
  { name: "properties", pgType: "JSONB", chType: "String" },
] as const;
...
export function extractClientContext(ctx: ClientContext | undefined): ClientContextColumns {
  if (!ctx) return { referer: undefined, user_agent: undefined, locale: undefined, rest: null };
...
export function eventToRow(event: NextlyticsEvent): Record<ColumnName, unknown> {
  ...
  const clientCtx = extractClientContext(event.clientContext);
  ...
  return {
    ...
    client_context: clientCtx.rest ? JSON.stringify(clientCtx.rest) : null,
...
export function generateChCreateTableSQL(database: string, tableName: string): string {
  const cols = tableColumns.map((c) => `    ${c.name} ${c.chType}`).join(",\n");
